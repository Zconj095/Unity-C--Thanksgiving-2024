...
public class RootChakraParticleSystem : MonoBehaviour
{
    public Color chakraColor = Color.red; // Red for Root Chakra
    public float chakraTransparency = 1.0f; // Fully opaque by default
    public float particleSize = 0.05f; // Size of individual particles (small size)
    ...
    void SetupChakraParticleSystem()
    {
        ...
        main.loop = true;
        main.startLifetime = Mathf.Infinity; // Infinite lifetime so particles don't disappear
        ...
        // You can customize the particle shape if needed (optional)
    }
}
